...
Provider failures are precise observations + feedback. At most three different
model routes per request; never fail over after the first streaming byte.
...
def chat(state, body):
    ...
    for model in candidates[:3]:
        config = state['selector'].select(model['id'])
        ...
        try:
            if req.get('stream'):
                ...
                chunks = wire.stream_chunks(events, config)
                try:
                    first = next(chunks)
